...
export const getAllShopsByEventIdQuery = async(array) =>{
    try {
        ...
        SELECT s.*, 
            CASE 
                WHEN s.status = 'occupied' THEN ra.end_date 
                ELSE NULL 
            END AS end_date
        FROM shops s
        LEFT JOIN (
    ...
    try {
        ...
        if (flag === 'shop') {
            ...
            SELECT 
                s.id AS shop_id, 
                s.shop_number,
                CASE WHEN s.status = 'vacant' THEN 'no' ELSE 'yes' END AS rented,
                CASE WHEN s.status = 'vacant' THEN 'yes' ELSE 'no' END AS vacant
            FROM 
                shops s
            LEFT JOIN 
            ...
            SELECT 
                ...
                COUNT(DISTINCT CASE WHEN s.status = 'vacant' THEN NULL ELSE s.id END) AS occupied,
                COUNT(DISTINCT CASE WHEN s.status = 'vacant' THEN s.id ELSE NULL END) AS vacant
            FROM 
                (SELECT 
                    ADDDATE(CONCAT(YEAR(CURDATE()), '-01-01'), INTERVAL m MONTH) AS month
                FROM 
            ...
            LEFT JOIN 
                shops s ON s.id = ra.shop_id AND s.event_id = ?
            GROUP BY
